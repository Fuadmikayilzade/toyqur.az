...
import { ArrowLeft, X, Heart, Star, MapPin, Phone, Share2, MessageCircle, Play, ExternalLink, Calendar, CheckCircle, Store, Instagram, Eye, ChevronLeft, ChevronRight, FileText } from "lucide-react";
...
import { CATEGORY_FILTERS, parseFilterMeta } from "@/data/categoryFilters";
...
const parseServiceMeta = (desc: string | null) => {
  if (!desc) return { cleanDesc: "", phones: [], address: "", logoUrl: "", instagram: "", district: "", capacity: "", amenities: [] as string[], cuisines: [] as string[], whatsapp: "", menuPdfUrl: "" };
  const sepIdx = desc.indexOf("\n---\n");
  const cleanDesc = sepIdx === -1 ? desc : desc.slice(0, sepIdx).trim();
  if (sepIdx === -1) return { cleanDesc, phones: [], address: "", logoUrl: "", instagram: "", district: "", capacity: "", amenities: [] as string[], cuisines: [] as string[], whatsapp: "", menuPdfUrl: "" };
  const metaLines = desc.slice(sepIdx + 5).split("\n");
  const phones: string[] = [];
  let address = "", logoUrl = "", instagram = "", district = "", capacity = "", menuPdfUrl = "";
  ...
  for (const line of metaLines) {
    ...
    } else if (line.startsWith("MenuPDF:")) {
      menuPdfUrl = line.slice(8).trim();
    }
  }
  let whatsapp = "";
  for (const line of metaLines) {
  ...
  return { cleanDesc, phones, address, logoUrl, instagram, district, capacity, amenities, cuisines, whatsapp, menuPdfUrl };
};
...
const FilterBadges = ({ description, category }: { description: string | null; category: string }) => {
  const catFilters = CATEGORY_FILTERS[category] || [];
  if (!catFilters.length) return null;

  const saved = parseFilterMeta(description, category);
  const badges: { label: string; value: string; emoji: string }[] = [];
  ...
  catFilters.forEach(field => {
    const val = saved[field.key];
    if (!val || val === false || (Array.isArray(val) && val.length === 0)) return;
    const emoji = FIELD_EMOJIS[field.key] || "●";

    if (typeof val === "boolean") {
      badges.push({ label: field.label, value: "Bəli", emoji });
    } else if (Array.isArray(val)) {
      val.forEach(v => {
        const opt = field.options?.find(o => o.value === v);
        if (opt) badges.push({ label: field.label, value: opt.label, emoji });
      });
    } else {
  ...
  return (
    <div className="mt-4 rounded-2xl p-5" style={{
      ...
      <h3 className="font-serif font-semibold text-foreground mb-3 text-sm flex items-center gap-2">
        <span style={{ color: "hsl(16 38% 44%)" }}>✦</span>
        Xüsusiyyətlər
      </h3>
      <div className="flex flex-wrap gap-2">
        {badges.map((b, i) => (
          <div
            ...
            style={{
              background: "linear-gradient(135deg, hsl(28 38% 97%) 0%, hsl(24 32% 93%) 100%)",
              border: "1px solid hsl(25 30% 84%)",
              color: "hsl(20 20% 25%)",
              boxShadow: "0 1px 3px hsl(25 18% 70% / 0.15)",
            }}
          >
            <span>{b.emoji}</span>
            <span style={{ color: "hsl(20 15% 52%)" }}>{b.label}:</span>
            <span className="font-semibold" style={{ color: "hsl(16 38% 36%)" }}>{b.value}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen">
            ...
            <div className="lg:col-span-3">
              ...
              {address && (() => {
                ...
                if (coordMatch) {
                  ...
                  if (isEmbeddable) {
                    embedSrc = address.includes("output=embed") ? address : address + (address.includes("?") ? "&output=embed" : "?output=embed");
                  }
                } else {
                  ...
                  <div className="mt-6 rounded-2xl overflow-hidden" style={cardStyle}>
                    <div className="p-4 flex items-center justify-between border-b border-border">
                      <h3 className="font-serif font-semibold text-foreground flex items-center gap-2">
                        <MapPin className="w-4 h-4 text-primary" />{t("mapTitle")}
                      </h3>
                      <a href={externalHref} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-1.5 text-sm font-medium text-primary hover:underline">
                        <ExternalLink className="w-3.5 h-3.5" />{t("mapBtn")}
                      </a>
                    </div>
                    {embedSrc ? (
                      <div style={{ height: 260 }}>
                        <iframe title="location-map" width="100%" height="260"
              ...
              {/* Description — only show if vendor filled it in */}
              {cleanDesc && cleanDesc.trim().length > 0 && (
              ...
              {/* Category filter badges — only show if vendor selected filters */}
              <FilterBadges description={service.description} category={service.category} />
            </div>
            ...
            <div className="lg:col-span-2">
              <div className="sticky top-24">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <h1 className="text-xl md:text-2xl font-serif font-bold text-foreground mb-1">{service.title}</h1>
                      {service.location && (
                      ...
                      <Link to={`/store/${service.vendor_id}`}
                        ...
                        style={{
                          ...
                          color: "#fff", border: "1px solid hsl(16 38% 40%)",
                  ...
                  <div className="flex gap-2 flex-shrink-0 relative">
                    ...
                    <div className="relative">
                      <button onClick={handleShare} className="w-10 h-10 rounded-full border border-border flex items-center justify-center hover:border-primary/30 transition-colors">
                        <Share2 className="w-4 h-4 text-foreground/50" />
                      </button>
                      {shareOpen && (
                        <>
                          <div className="fixed inset-0 z-40" onClick={() => setShareOpen(false)} />
                          <div className="absolute right-0 top-12 z-50 rounded-2xl p-4"
                            style={{
                              background: "linear-gradient(145deg, hsl(30 42% 99%) 0%, hsl(24 35% 96%) 100%)",
                              border: "1px solid hsl(25 28% 85%)",
                              boxShadow: "0 8px 32px hsl(20 20% 60% / 0.18)",
                              minWidth: 200,
                            }}>
                            <p className="text-xs font-semibold tracking-widest uppercase text-muted-foreground mb-3 text-center">{t("share")}</p>
                            <div className="flex items-center justify-center gap-3">
                              <a href={`https://wa.me/?text=${shareText}%20${shareLink}`} target="_blank" rel="noopener noreferrer" onClick={() => setShareOpen(false)} className="group flex flex-col items-center gap-1.5">
                                <div className="w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm transition-transform group-hover:scale-110" style={{ background: "linear-gradient(145deg, #25d366, #128c7e)" }}>
                              ...
                              <a href={`https://t.me/share/url?url=${shareLink}&text=${shareText}`} target="_blank" rel="noopener noreferrer" onClick={() => setShareOpen(false)} className="group flex flex-col items-center gap-1.5">
                                <div className="w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm transition-transform group-hover:scale-110" style={{ background: "linear-gradient(145deg, #29b6f6, #0277bd)" }}>
                              ...
                              <button onClick={async () => { try { await navigator.clipboard.writeText(shareUrl); toast.success(t("igCopy")); } catch { toast.error(t("error")); } setShareOpen(false); }} className="group flex flex-col items-center gap-1.5">
                                <div className="w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm transition-transform group-hover:scale-110" style={{ background: "linear-gradient(145deg, #f9ce34, #ee2a7b, #6228d7)" }}>
                              ...
                              <button onClick={async () => { try { await navigator.clipboard.writeText(shareUrl); toast.success(t("copied")); } catch { toast.error(t("error")); } setShareOpen(false); }} className="group flex flex-col items-center gap-1.5">
                                <div className="w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm transition-transform group-hover:scale-110" style={{ background: "linear-gradient(145deg, hsl(25 35% 55%), hsl(20 30% 38%))" }}>
                                  <svg viewBox="0 0 24 24" className="w-5 h-5 fill-none stroke-white stroke-2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg>
                                </div>
                  ...
                  <div className="mb-5 rounded-xl p-4 space-y-3" style={{ background: "hsl(28 35% 95%)", border: "1px solid hsl(25 26% 87%)" }}>
                    ...
                    {amenities.length > 0 && (
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {amenities.map((a) => (
                            <span key={a} className="text-xs px-2.5 py-1 rounded-full font-medium" style={{ background: "hsl(142 35% 90%)", color: "hsl(142 40% 28%)" }}>{a}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    {cuisines.length > 0 && (
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {cuisines.map((c) => (
                            <span key={c} className="text-xs px-2.5 py-1 rounded-full font-medium" style={{ background: "hsl(25 35% 88%)", color: "hsl(20 30% 30%)" }}>{c}</span>
                          ))}
                        </div>
                      </div>
                    )}
                ...
                <div className="flex flex-col gap-3">
                  <Button size="lg" className="rounded-xl w-full text-white" onClick={() => setMessageModalOpen(true)}
                    style={{ background: "hsl(16 38% 44%)" }}>
                    <MessageCircle className="w-5 h-5 mr-2" />
                    Mesaj Göndər
                  </Button>
                  <Button size="lg" className="rounded-xl w-full text-white" style={{ background: "hsl(142 60% 42%)" }} asChild>
                    <a href={`https://wa.me/${waNumber}?text=${waAutoMsg}`} target="_blank" rel="noopener noreferrer">
                      <MessageCircle className="w-5 h-5 mr-2" />{t("contactBtn")}
                    </a>
                  </Button>
                  {phones.map((phone, i) => (
                    <Button key={i} size="lg" variant="outline" className="rounded-xl w-full" asChild>
                      <a href={`tel:${phone.replace(/\s/g, "")}`}><Phone className="w-5 h-5 mr-2" />{phone}</a>
                    </Button>
                  ))}
                  {phones.length === 0 && (
                    <Button size="lg" variant="outline" className="rounded-xl w-full" asChild>
                      <a href="tel:+994104195344"><Phone className="w-5 h-5 mr-2" />+994 10 419 53 44</a>
                    </Button>
                  )}
                  {instagram && (
                    <Button size="lg" variant="outline" className="rounded-xl w-full" asChild>
      ...
      {lightboxOpen && images.length > 0 && (
        <ImageLightbox images={images} initialIndex={safeIdx} onClose={() => setLightboxOpen(false)} />
      )}
        ...
        <div className="pb-6" style={{ background: "hsl(28 38% 98%)" }}>
          <div className="container mx-auto px-4">
            <DetailSlider
              ...
              items={similarServices} accentColor="hsl(16 38% 48%)"
      ...
      {mixedServices.length > 0 && (
        <div className="pb-16" style={{ background: "hsl(28 38% 98%)" }}>
          <div className="container mx-auto px-4">
            <DetailSlider title={t("otherProducts")} items={mixedServices}
              accentColor="hsl(25 35% 65%)" viewAllLink="/categories" />
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 md:p-6"
          ...
          <div className="relative w-full max-w-3xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="relative rounded-2xl overflow-hidden shadow-2xl" style={{ background: "hsl(20 15% 10%)" }}>
              ...
              {menuImages.length > 1 && (
                <>
                  <button onClick={() => setMenuActiveIdx(i => (i - 1 + menuImages.length) % menuImages.length)}
                    className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center shadow-lg"
                    style={{ background: "hsl(16 38% 38%)" }}>
                    <ChevronLeft className="w-4 h-4 text-white" />
                  </button>
                  <button onClick={() => setMenuActiveIdx(i => (i + 1) % menuImages.length)}
            ...
            {menuImages.length > 1 && (
              <div className="flex items-center gap-3 mt-3">
                {logoUrl && (
                  <div className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0 border-2" style={{ borderColor: "hsl(16 38% 48%)" }}>
                    <img src={logoUrl} alt="logo" className="w-full h-full object-cover" />
                  </div>
                )}
