...
// Video thumbnail — cross-platform (iOS, Android, Web)
const VideoThumb = ({ src }: { src: string }) => {
  ...
  const handleLoaded = () => {
    ...
    if (!v) return;
    // Seek to 1st second to get a meaningful frame on all platforms
    if (v.duration && v.duration > 0) {
      v.currentTime = Math.min(0.5, v.duration * 0.05);
    }
  };

  const handleSeeked = () => setHasFrame(true);
